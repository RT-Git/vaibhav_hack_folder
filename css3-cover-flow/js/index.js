...
window.cancelAnimFrame = (function () {
    return window.cancelAnimationFrame || window.webkitCancelRequestAnimationFrame || window.mozCancelRequestAnimationFrame || window.oCancelRequestAnimationFrame || window.msCancelRequestAnimationFrame || window.clearTimeout(renderTimeout);
}());
/**
...
var coverflow = {
    ...
    , bind: function (coverflowDOM) {
        ...
        var currentId = null;
        var delta = 1;
        var text = document.getElementById("arc");
                ...
                if (Math.abs(delta) > 0.01) {
                    for (var i = 0; i < numItems; i++) {
                        ...
                        if (currentId > i) {
                            ...
                            console.log("Now: " + currentId);
                            //gives the ID of the current element. You can use it to set the number of followers intead
                            text.innerHTML = currentId;
                        }
                    }
                }
            }
...
// change it to number of items from your API.
var numItems = 50;
var types = [
    '<li><a href="#"><i class="devicon-csharp-plain"></i></a></li>'
    // Preferaby use SVG. 
];
for (var i = 0, markup = ''; i < numItems; i++) {
    // this one adds its randomly. Change it to suit your needs
    markup += types[Math.floor(Math.random() * types.length)];
}
